...
struct Node
{
	...
	Node *l, *r;
	bool rev;
	Node(int _valor) : rev(false), sum(_valor), valor(_valor), priority((rand() << 16) ^ rand()), size(1), l(nullptr), r(nullptr) {}
...
struct Treap
{
	...
	{
		if(t == nullptr) return -1;//nao esta na treap, botar valor que noa esta no array...
		propagate(t);
