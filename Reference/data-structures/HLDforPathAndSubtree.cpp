...
caminho de nxt[v] até v, Em que nxt[v] está no inicio da
cadeia da HLD. e o caminho de nxt[v] até v faz parte da
cadeia que começa em nxt[v].

Assim, podemos processar queries rapidamente em caminhos
e subarvores usando a mesma segment tree.

Bonus: para uma query de mudança de raiz: se a raiz atual
for v e a consulta for na subarvore de u, então, se u for 
ancestral de v a resposta é a consulta da arvore
total menos a consulta da subarvore enraizada pelo filho
de u que é ancestral de v, caso contrario a consulta é
normal como se a raiz da arvore nunca tivesse mudado.s
obs: Para encontrar o filho de u que é ancestral de v
