...
int addEdge(int u, int v)
{
    ...
    // aplicação o grafo G sera igual ao transposto
...
void mountDAG()
{
    ...
    for(int v = 1; v < comp; v++)
        if(!cor[v])
            topSort(v);
    // não inverter ts, pois precisamos da ordenação
    // topologica ao contrario
}

void assignment() // encontrar uma atribuição (TREU ou FALSE) para as proposições
{
    if(!twoSat()) return;
    mountDAG();
    ...
    for(int &v : ts)
        for(int &u : C[v])
            if(value[u >> 1] == -1) //   u / 2 é a proposição
                value[u >> 1] = (u & 1 ? 1 : 0);
