...
// 0: padrão está no sufixo
// 1: o padrão é lexicograficamente maior que o sufixo k
//-1: o padrão é lexicograficamente menor que o sufixo k
// O(|p|)
...
void buildLcp()
{
	...
	// sufixo que começa na posiçao i da strig
		int pi = c[i]; 
		int j = sa[pi - 1];
		while(s[i + k] == s[j + k]) k++;
		lcp[pi] = k;
